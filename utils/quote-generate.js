...
const emojiImageData = require('./emoji-image')
...
class QuoteGenerate {
    ...
    for (let index = 0; index < styledWords.length; index++) {
      ...
      if (styledWord.emoji) {
        if (styledWord.customEmojiId && customEmojiStickers[styledWord.customEmojiId]) {
          ...
          const emojiImageBase = emojiImageData[styledWord.emoji.code]
          if (emojiImageBase) {
  ...
  // emojiBrand is no longer used — every emoji now renders in Apple style.
  // Kept as the last parameter so existing callers passing it don't break.
  async generate (backgroundColorOne, backgroundColorTwo, message, width = 512, height = 512, scale = 2, _emojiBrand) {
    if (!scale) scale = 2
